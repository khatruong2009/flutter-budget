...
class AppDesign {
  ...
  @Deprecated('Use getCardColor() instead')
  ...
  static List<LinearGradient> getChartGradients(BuildContext context) {
    return [getPrimaryGradient(context), getIncomeGradient(context), getExpenseGradient(context)];
  }

  @Deprecated('Use ElevatedCard instead')
  static Color getGlassSurface(BuildContext context, double opacity) {
    return Theme.of(context).brightness == Brightness.dark
  ...
  @Deprecated('Use ElevatedCard instead')
  static Color getGlassBorder(BuildContext context, double opacity) {
    return Theme.of(context).brightness == Brightness.dark
        ? Colors.white.withValues(alpha: opacity * 0.5)
        : Colors.white.withValues(alpha: opacity);
  }

  // Deprecated glassmorphism constants - kept for backward compatibility
...
/// GlassCard is a reusable card component with glassmorphism effect
/// featuring backdrop blur, semi-transparent background, and border highlights
/// @deprecated Use ElevatedCard instead for the modernized design system
@Deprecated('Use ElevatedCard instead')
class GlassCard extends StatefulWidget {
  /// The widget to display inside the card
  final Widget child;

  /// The blur intensity for the backdrop filter (default: 10.0)
  final double blur;

  /// The opacity of the card surface (default: 0.15)
  ...
  /// Optional border width (default: AppDesign.borderMedium)
  final double? borderWidth;

  /// Optional border opacity (default: AppDesign.glassBorderOpacity)
  ...
  const GlassCard({
    super.key,
    required this.child,
    this.blur = AppDesign.glassBlurLight,
    this.opacity = AppDesign.glassOpacity,
    this.borderRadius,
    this.padding,
    this.onTap,
    this.onLongPress,
...
class _GlassCardState extends State<GlassCard> {
  ...
  Widget build(BuildContext context) {
    final effectiveBorderRadius =
        widget.borderRadius ?? BorderRadius.circular(AppDesign.radiusL);
    final effectivePadding =
        widget.padding ?? const EdgeInsets.all(AppDesign.spacingM);
    final effectiveBorderWidth = widget.borderWidth ?? AppDesign.borderMedium;
    final effectiveBorderOpacity =
        widget.borderOpacity ?? AppDesign.glassBorderOpacity;

    // Wrap BackdropFilter in RepaintBoundary to optimize blur performance
        ...
        child: BackdropFilter(
          ...
          child: Container(
            decoration: BoxDecoration(
              color: AppDesign.getGlassSurface(context, widget.opacity),
              borderRadius: effectiveBorderRadius,
              border: Border.all(
                color:
                    AppDesign.getGlassBorder(context, effectiveBorderOpacity),
    ...
    // Add hover effects for desktop platforms
    if (MicroInteractions.shouldEnableHover() &&
